...
# creating some sample transactions in order to check for validity (some are fraudulent)
...
state = {u'Alice':50, u'Bob':50}  # specify the initial state
...
# specify genesis block as link element to ensuing blockchain
chain = [genesisBlock]

# for new block, we collect transactions, create a header, hash it, and add to chain
...
# process transaction buffer into a set of blocks
...
# check validity of the states
checkChain(chain)

# check integrity of the chain and create the current state
...
# prints a check for block validity and updates state if valid block
